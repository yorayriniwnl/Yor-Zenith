...
/*
  ...
  - Keeps demo fallback data disabled for production analysis.
  - Shows pricing assumptions when a feasibility result is not linked.
...
const EMPTY_PROJECT: StoredProjectData = {
  city: null,
  state: null,
  latitude: null,
  longitude: null,
...
const loadStoredProjectData = (): StoredProjectData | null => {
  ...
  const annualProduction =
    parseNumeric(getIn(fsResult, ["annualProduction"])) ??
    sumField(monthlyData, "production");
  const annualConsumption =
    parseNumeric(getIn(fsResult, ["annualConsumption"])) ??
    sumField(monthlyData, "consumption");
  ...
  return new Promise((resolve, reject) => {
    ...
    video.onloadeddata = () => {
      if (!video.videoWidth || !video.videoHeight) {
        cleanup();
        reject(new Error("The video did not contain a readable frame."));
        return;
      }
...
interface RooftopAnalysisResult {
  roofAreaSqFt: number;
  recommendedKW: number;
}

const normalizeRooftopAnalysis = (value: unknown): RooftopAnalysisResult | null => {
  if (!value || typeof value !== "object") return null;
  const record = value as Record<string, unknown>;
  const roofAreaSqFt = Number(record.roofAreaSqFt);
  const recommendedKW = Number(record.recommendedKW);
  if (!Number.isFinite(roofAreaSqFt) || roofAreaSqFt <= 0) return null;
  if (!Number.isFinite(recommendedKW) || recommendedKW <= 0) return null;
  return { roofAreaSqFt, recommendedKW };
};

async function analyzeRooftop(frame: string, signal?: AbortSignal): Promise<RooftopAnalysisResult | null> {
  const res = await fetch("/api/rooftop-vision", {
    ...
    signal,
    body: JSON.stringify({ image: frame, mimeType: "image/jpeg" }),
  });

  if (!res.ok) {
    throw new Error("Rooftop analysis request failed.");
  }

  const data: unknown = await res.json();
  if (!data || typeof data !== "object") return null;

  const analysis = "analysis" in data ? data.analysis : null;
  const normalizedAnalysis = normalizeRooftopAnalysis(analysis);
  if (normalizedAnalysis) return normalizedAnalysis;

  const text = "text" in data && typeof data.text === "string" ? data.text : "";
  return (() => {
    try {
      const parsed = JSON.parse(text);
      return normalizeRooftopAnalysis(parsed);
    } catch {
      return null;
    }
  })();
...
export default function RooftopPage() {
  ...
  },[curHW.efficiency,energy,initialDate,metrics.kwh,metrics.kw,panelNormal,panelTilt,panels,projectData,siteLatitude,solarAngles.elevation,solarDay,sunDirection]);
  const roi = useMemo(
  ...
  const startScan=useCallback(async ()=>{
    ...
    try {
      ...
      const ai = await analyzeRooftop(frame, controller.signal);
      if (!ai) throw new Error("The rooftop image could not be interpreted.");

      setProjectData((prev) => ({
        ...(prev ?? EMPTY_PROJECT),
        roofAreaSqFt: ai.roofAreaSqFt,
        recommendedKW: ai.recommendedKW,
      }));
    } catch {
      if (controller.signal.aborted) return;
      setScanError("Rooftop analysis failed. Check the video and try again.");
      setAnalyzing(false);
      return;
    }
    setAnalyzing(false);
  ...
  const handleFile=useCallback((f:File)=>{
    ...
    if (!f.type.startsWith("video/")) {
      setScanError("Please upload a supported video file.");
      return;
    }
    if (f.size === 0 || f.size > 200 * 1024 * 1024) {
      setScanError("Video files must be between 1 byte and 200 MB.");
      return;
    }
    setVideo(f);
  },[reset]);
  ...
  const handleInstallerQuote = useCallback(() => {
    const installerEmail = process.env.NEXT_PUBLIC_INSTALLER_EMAIL;
    if (!installerEmail) {
      setCtaNotice("Installer quote routing is not configured. Set NEXT_PUBLIC_INSTALLER_EMAIL before launch.");
      return;
    }

    const subject = encodeURIComponent(`Zenith installer quote · ${curSite.name}`);
    const body = encodeURIComponent([
      `Site: ${curSite.name}`,
      `System: ${metrics.kw.toFixed(1)} kW`,
      `Annual production estimate: ${metrics.kwh.toLocaleString("en-IN")} kWh`,
      `Estimated annual savings: ${formatINR(metrics.sav)}`,
      `Payback estimate: ${metrics.pb} years`,
      "",
      "Please contact me about an installer quote.",
    ].join("\n"));

    window.location.href = `mailto:${installerEmail}?subject=${subject}&body=${body}`;
  },[curSite.name,metrics.kwh,metrics.kw,metrics.pb,metrics.sav]);
  ...
  if (!projectData && !done) {
    return (
      <main className="rooftop-empty-state" style={{minHeight:"100vh",background:T.bg,color:T.text,padding:"96px 20px 40px",fontFamily:"'Inter','SF Pro Display',system-ui,sans-serif"}}>
        <section style={{maxWidth:"720px",margin:"0 auto",...cs,padding:"32px"}}>
          <p style={{fontSize:"11px",fontWeight:800,letterSpacing:"0.16em",textTransform:"uppercase",color:T.teal}}>Rooftop feasibility</p>
          <h1 style={{fontSize:"clamp(32px,6vw,58px)",lineHeight:1,letterSpacing:"-0.06em",margin:"14px 0 16px"}}>Start with a real roof input.</h1>
          <p style={{fontSize:"15px",lineHeight:1.7,color:T.textMid,maxWidth:"560px",margin:0}}>
            Upload a short roof video or complete Service 1 first. Zenith will show a preliminary planning estimate only after an input is analyzed.
          </p>
          ...
          <div style={{display:"flex",gap:"10px",flexWrap:"wrap",marginTop:"26px"}}>
            <button type="button" onClick={()=>fileRef.current?.click()} style={{...btnPrimary,display:"inline-flex",alignItems:"center",gap:"8px"}}>
              <Upload size={15}/>{video ? "Replace roof video" : "Choose roof video"}
            </button>
            <button type="button" onClick={startScan} disabled={!video || analyzing} style={{...btnSecondary,opacity:video && !analyzing ? 1 : 0.5,cursor:video && !analyzing ? "pointer" : "not-allowed"}}>
              {analyzing ? "Analyzing frame…" : "Run preliminary scan"}
            </button>
          </div>
          {video && <p style={{fontSize:"12px",color:T.textMid,margin:"16px 0 0"}}>{video.name} · ready for one-frame AI analysis</p>}
          {scanError && <p role="alert" style={{fontSize:"12px",color:"#ef4444",margin:"16px 0 0"}}>{scanError}</p>}
        </section>
      </main>
    );
  }
      ...
      >
        <section style={{maxWidth:"1320px",margin:"0 auto",padding:"10px 28px 0"}}>
          ...
          <div style={{marginBottom:"44px"}}>
            <motion.div variants={fadeUp}>
              ...
              >
                Single-frame AI-assisted layout preview for preliminary solar planning - not a structural, electrical, or shading survey.
              </motion.p>
            </motion.div>
        ...
        <main style={{maxWidth:"1320px",margin:"0 auto",padding:"28px 28px 64px",
          ...
          <motion.div variants={fadeUp}>
            <div style={{...cs, boxShadow:"0 4px 24px rgba(0,0,0,0.3)"}}>
              ...
              <div style={{padding:"0 18px 18px"}}>
                ...
                {scanError && (
                  <div role="alert" style={{marginTop:"10px",padding:"10px 12px",borderRadius:"8px",
                    border:"1px solid rgba(248,113,113,0.35)",background:"rgba(127,29,29,0.18)",
          ...
          >
            ...
            <div className="rooftop-hardware-grid" style={{padding:"10px 18px",display:"grid",gap:"10px"}}>
          ...
          <motion.div variants={fadeUp}>
            <div style={{...cs,boxShadow:"0 4px 24px rgba(0,0,0,0.3)",overflow:"hidden"}}>
              ...
              <div style={{padding:"0 18px 18px"}}>
                <div className="rooftop-scene" style={{borderRadius:"18px",overflow:"hidden",background:"linear-gradient(180deg,rgba(3,12,33,0.96),rgba(4,10,22,0.88))"}}>
                  <Suspense fallback={
          ...
          <motion.div
            ...
            className="rooftop-financial-grid"
          ...
          >
            ...
            <div className="rooftop-report-kpis" style={{display:"grid",gap:"12px",marginBottom:"24px"}}>
              {[
                    ...
                    <div style={{position:"relative",zIndex:1}}>
                      ...
                      <div style={{display:"flex",gap:"10px",justifyContent:"center",flexWrap:"wrap"}}>
                        ...
                        <motion.button
                          ...
                          <FileText size={14}/>{generating ? "Generating Report…" : "Download Full Report"}
                        </motion.button>
                      </div>
                      {ctaNotice && <p role="status" style={{margin:"12px 0 0",fontSize:"12px",color:T.amber,textAlign:"center"}}>{ctaNotice}</p>}
                    </div>
                  </div>
            </motion.div>
          )}
