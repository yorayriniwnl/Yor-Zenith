 ...
 * - Illustrative telemetry preview (not a live grid connection)
...
const JURISDICTION_REGISTRY: Record<string, JurisdictionData> = {
  // State incentives stay at zero until each DISCOM/state programme has a
  // versioned source, eligibility rules, and effective date in the registry.
  "Delhi (NCT)": { state: "Delhi", irradiance: 5.1, tariff: 8.5, meta: "State incentive verification required", rule: () => 0 },
  "Mumbai": { state: "Maharashtra", irradiance: 5.3, tariff: 10.2, meta: "State incentive verification required", rule: () => 0 },
  "Bengaluru": { state: "Karnataka", irradiance: 5.4, tariff: 8.15, meta: "State incentive verification required", rule: () => 0 },
  "Chennai": { state: "Tamil Nadu", irradiance: 5.5, tariff: 7.8, meta: "State incentive verification required", rule: () => 0 },
  "Bhubaneswar": { state: "Odisha", irradiance: 5.4, tariff: 6.2, meta: "State incentive verification required", rule: () => 0 },
  "Lucknow": { state: "Uttar Pradesh", irradiance: 5.1, tariff: 6.8, meta: "State incentive verification required", rule: () => 0 },
  "Other / Generic": { state: "Other", irradiance: 4.8, tariff: 7.5, meta: "National Baseline", rule: () => 0 }
};

const CENTRAL_SUBSIDY_PER_KW = {
  firstTwoKw: 30_000,
  additionalKw: 18_000,
  capKw: 3,
  capAmount: 78_000,
...
function getCentralSubsidy(kw: number): number {
  const firstTwoKw = Math.min(Math.max(kw, 0), 2) * CENTRAL_SUBSIDY_PER_KW.firstTwoKw;
  const additionalKw = Math.min(Math.max(kw - 2, 0), 1) * CENTRAL_SUBSIDY_PER_KW.additionalKw;
  return Math.min(
    firstTwoKw + additionalKw,
    CENTRAL_SUBSIDY_PER_KW.capAmount,
  );
}
...
const useAnimatedNumber = (target = 0, duration = 1200) => {
  ...
  useEffect(() => {
    ...
    const step = (ts: number) => {
      ...
      setValue(Math.round(MathCore.lerp(from, target, MathCore.easeOutExpo(t))));
...
/* ================================
   Engine core
   ================================ */
function calculateSystemData(kw: number, jurisdictionKey: string, isCommercial: boolean, batteryKwh: number =0) {
  kw = Math.max(0.1, +kw);
...
export default function GovSubsidyPage() {
  ...
  [kw, jurisdiction, isCommercial, batteryKwh]
);
...
const typeText = useCallback((text: string) => {
  ...
  textIntervalRef.current = interval;
}, []);
  ...
  try {
    ...
    const res = await fetch("/api/gemini-subsidy-explanation", {
      method: "POST",
      headers: {
  ...
  subsidyCoverage: subsidyPct
    ...
    const text =
      data && typeof data === "object" && "text" in data && typeof data.text === "string"
        ? data.text.trim()
        : "";
    if (!res.ok) throw new Error(text || "Unable to generate explanation at the moment.");
    const explanation = text || "Unable to generate explanation at the moment.";
    typeText(explanation);

  } catch (error) {
    if (controller.signal.aborted || requestId !== aiRequestRef.current) return;
    console.error("AI error:", error);
    typeText("Unable to generate explanation at the moment.");
  } finally {
    if (requestId === aiRequestRef.current) setAiLoading(false);
  }
}, [calc, jurisdiction, kw, subsidyPct, typeText]);
...
useEffect(() => {
  ...
  const handleLock = () => {
    ...
    timeoutRefs.current.t2 = setTimeout(() => {
      window.localStorage.setItem(
        "zenith.subsidy-scenario.v1",
        JSON.stringify({ jurisdiction, kw, batteryKwh, isCommercial, savedAt: new Date().toISOString() }),
      );
      setLocked(true);
      setLockAnim(0);
    }, 2200);
          ...
          <aside className="z-panel input-panel">
            ...
            <div className="panel-content scroll-y">
              ...
              <div className="input-group">
                <label htmlFor="subsidy-jurisdiction">Region / Dispatch Center</label>
<select id="subsidy-jurisdiction" value={jurisdiction} onChange={(e) => {
  ...
  setJurisdiction(e.target.value);
              ...
              <div className="input-group">
                <label htmlFor="subsidy-capacity">PV Array Capacity: <span className="hl">{kw} kWp</span></label>
                <input
  id="subsidy-capacity"
  type="range"
  ...
  aria-label="PV array capacity in kilowatts peak"
  className="z-slider"
/>
              </div>
              ...
              <div className="input-group">
  <label htmlFor="subsidy-battery">
    <span id="subsidy-battery-label">
...
onChange={(e) => {
  ...
  setBatteryKwh(Number(e.target.value));
    ...
    aria-labelledby="subsidy-battery-label"
  ...
  />
              ...
              <button type="button" className={`z-btn massive ${locked ? "locked" : lockAnim === 1 ? "loading" : ""}`} onClick={handleLock}>
                <span className="icon">{locked ? "✓" : lockAnim === 1 ? "⟳" : "⚡"}</span>
                <span className="text">{locked ? "Scenario Saved Locally" : lockAnim === 1 ? "Saving Scenario..." : "Save Scenario Locally"}</span>
              </button>
            </div>
          </aside>
...
<ul className="ai-reasons">
  <li>• Central CFA is estimated using a capped residential slab model</li>
  <li>• State incentives are excluded until local eligibility is verified</li>
  <li>• Larger systems may receive no additional central CFA beyond the cap</li>
</ul>

  <p className="ai-sub">
    ⚠️ Subsidy rules change frequently. These figures are estimates and must
    be verified against current MNRE and DISCOM rules before purchase.
  </p>
</div>
                    </div>
                  </div>
...
.hs-gauge {
  ...
  justify-content: center;
        ...
        @media (max-width: 1024px) {
          ...
          .hs-row { grid-template-columns: repeat(2, minmax(0, 1fr)); }
          .z-header { gap: 16px; }
        }

        @media (max-width: 640px) {
          .z-header { flex-direction: column; align-items: stretch; padding: 18px; border-radius: 18px; }
          .z-brand { gap: 10px; }
          .toggle-pill { width: 100%; }
          .toggle-pill button { flex: 1; padding: 10px 8px; font-size: 12px; }
          .panel-content, .z-board-content { padding: 20px; }
          .hs-row { grid-template-columns: minmax(0, 1fr); }
          .hs-card { padding: 18px; }
          .feature-split { gap: 14px; }
        }
