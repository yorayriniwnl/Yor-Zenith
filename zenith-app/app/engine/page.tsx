...
const MODULES: ModuleConfig[] = [
  {
    ...
    name: "F.S SCORE",
    shortName: "F.S SCORE",
    desc: "Models rooftop size, electricity usage, ROI, payback period, and lifetime savings to create a reviewable feasibility score.",
    angle: Math.PI * 1.92,
    orbitRadius: 3.1,
    color: "#34d399",
    colorHex: 0x34d399,
    ...
    cardIcon: "layers",
    cardDesc: `1. The user uploads an electricity bill or enters monthly usage details.
2. Zenith extracts or accepts the inputs used by the model.
3. The engine estimates system size, generation, cost, and savings.
4. It calculates payback and supporting financial indicators.
5. A feasibility score summarizes the modeled case.
6. Optional AI assistance explains the assumptions and trade-offs.`,
  },
  {
    id: 1,
    name: "FUTURE POTENTIAL",
    shortName: "FUTURE POTENTIAL",
    desc: "Long-term electricity inflation modeling, carbon offset projections, and ROI forecasting across reviewable scenarios.",
    angle: Math.PI * 0.18,
    orbitRadius: 3.55,
    color: "#00d4aa",
    colorHex: 0x00d4aa,
    ...
    cardIcon: "chart",
    cardDesc: `1. The system takes outputs from the feasibility engine as a starting point.
2. It models a 20-25 year planning horizon.
3. Tariff growth, degradation, and operating assumptions are made visible.
4. Financial metrics such as NPV and IRR are calculated.
5. The user compares returns and cash flow across scenarios.
6. Optional AI assistance highlights sensitivity and uncertainty.`,
  },
  {
    id: 2,
    name: "GOV. SUBSIDY",
    shortName: "GOV. SUBSIDY",
    desc: "Estimates configured PM Surya Ghar subsidy slabs and surfaces the eligibility details that still need official confirmation.",
    angle: Math.PI * 0.92,
    orbitRadius: 3.05,
    color: "#3b8fff",
    colorHex: 0x3b8fff,
    ...
    cardIcon: "document",
    cardDesc: `1. The user provides the details used for a policy estimate.
2. The engine applies configured central subsidy slabs.
3. State incentives remain separate until they are source-verified.
4. The estimated incentive is shown against the modeled system cost.
5. The user sees which details require MNRE, DISCOM, or installer confirmation.
6. Optional AI assistance explains how the estimate changes the scenario.`,
  },
  {
    id: 3,
    name: "ROOFTOP ANALYSIS",
    shortName: "ROOFTOP ANALYSIS",
    desc: "Creates an image-assisted first-pass screen for roof orientation, obstructions, usable area, and a preliminary panel layout.",
    angle: Math.PI * 0.65,
    orbitRadius: 3.25,
    color: "#a855f7",
    colorHex: 0xa855f7,
    ...
    cardIcon: "roof",
    cardDesc: `1. The user uploads a rooftop image for an initial screen.
2. The system analyzes visible geometry and likely obstructions.
3. It estimates usable area from the available visual evidence.
4. A preliminary panel layout is simulated for discussion.
5. The system provides an indicative generation range.
6. Final structural, electrical, shading, and installation checks remain with qualified professionals.`,
  },
];
      ...
      >
        <span
          style={{
            ...
            textShadow: "0 0 8px rgba(110,231,183,0.45)",
          }}
        >
      ...
      >
        <div
          style={{
            ...
            fontFamily: "Space Grotesk, sans-serif",
            textAlign: labelConfig.align,
            textShadow: "0 0 8px rgba(0,0,0,0.75)",
            textTransform: "uppercase",
          }}
        >
          <div style={{ fontSize: "14px", fontWeight: 800, letterSpacing: "0.14em" }}>
            {config.shortName}
          </div>
          <div style={{ color: "rgba(255,255,255,0.45)", fontSize: "10px", letterSpacing: "0.06em", marginTop: "4px" }}>
...
function OrbitalSystem({ hoveredId, onHover }: { hoveredId: number | null; onHover: (id: number | null) => void }) {
  const [frame, setFrame] = useState(() => ({
    ...
    positions: MODULES.map((m) => nodePosition(m.angle, m.orbitRadius ?? DEFAULT_ORBIT_RADIUS, 0, m.id)),
    time: 0,
  }));

  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    const angles = frame.angles.map((a) => a + ORBIT_SPEED);
    const positions = angles.map((a, i) =>
      nodePosition(a, MODULES[i].orbitRadius ?? DEFAULT_ORBIT_RADIUS, t, MODULES[i].id)
    );
    setFrame({ angles, positions, time: t });
  });
...
function SceneEnvironment() {
  const particlePos = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      arr[i * 3] = (unit - 0.5) * 36;
      arr[i * 3 + 1] = ((unit * 1.73) % 1 - 0.5) * 24;
      arr[i * 3 + 2] = ((unit * 2.41) % 1 - 0.5) * 24;
    }
    return arr;
  }, []);
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-[#06090f] text-gray-50 font-sans">
      ...
      >
        ...
        >
          From feasibility modeling to subsidy estimates and installer handoff -
          a clearer starting point for rooftop solar decisions.
        </p>
        <div
          style={{
            marginTop: "22px",
      ...
      >
        ...
        >
          Explore a module above; details are listed below
        </div>
      </div>
